			...
			<div class="container" id="content">
				<h2>질문과 답변</h2>
					<form action="update.do" method="post" enctype="multipart/form-data" id="update_form">
						<input type="hidden" name="nb_num" value="${boardVo.nb_num}">
						<div class="form-group col-md-12">
							<label for="nb_subject">제목</label> <input type="text" class="form-control" name="nb_subject" id="nb_subject" value="${boardVo.nb_subject }">
						</div>
						<div class="form-group col-md-12">
							<label for="nb_content">내용</label>
							<textarea class="form-control" rows="5" name="nb_content" id="nb_content">${boardVo.nb_content }</textarea>
						</div>
						<div class="form-group col-md-12">
							<label for="nb_filename">파일</label> <input type="file" name="nb_filename" id="exampleInputFile">
								<c:if test="${!empty boardVo.nb_filename}">
									<br>
									<span>(${boardVo.nb_filename})파일이 등록되었습니다 다시 업로드하면 기존 파일은 삭제됩니다</span>
							</c:if>
						</div>
						<div class="col-xs-12 text-center">
							<button type="submit" class="btn btn-info">수정</button>
							<button type="button" class="btn btn-warning" onclick="location.href='../board/list.do'">목록으로</button>
						</div>
				</form>
			</div>
	</div>
